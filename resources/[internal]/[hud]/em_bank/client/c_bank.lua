...
RegisterNUICallback("transfer_amount", function(data, cb)

    local response = exports["em_dal"]:bank_transfer_amount(data.amount, data.from_bank_account_id, data.to_bank_account_id, data.reason)
    if not response.result.successful then
        ...
        local temp_pending = exports["em_dal"]:bank_get_pending_transactions(data.from_bank_account_id)
        local temp_transactions = exports["em_dal"]:bank_get_transactions(data.from_bank_account_id)
        SendNUIMessage({pending = temp_pending, populate_pending = true})
        SendNUIMessage({transactions = temp_transactions, populate_transactions = true})
