local leftdoor, rightdoor= nil, nil, nil
...
AddEventHandler('em_dal:character_loaded', function(xPlayer)
	PlayerData = xPlayer
...
AddEventHandler('esx_JewelRobbery:policenotify', function()
	TriggerEvent("em_group_alerts:send_dispatch", "Law Enforcement", "Jewelry Store", "Someone is robbering the jewerly store!", 2)
end)
...
Citizen.CreateThread( function()
	while true do 
		...
		if IsInArea and not HasAlreadyEnteredArea then
			...
			shockingevent = false
			if Config.Closed and not (exports["em_jobs"]:get_num_law_enforcement_clocked_in() >= Config.MinPolice) and not policeclosed then
		...
		if Config.Closed and not (exports["em_jobs"]:get_num_law_enforcement_clocked_in() >= Config.MinPolice) and not storeclosed and not policeclosed then
			Citizen.Wait(1250)
		else
			Citizen.Wait(3250)
		end
...
Citizen.CreateThread( function()
	while true do 
		...
		while IsAbleToRob and (exports["em_jobs"]:get_num_law_enforcement_clocked_in() >= Config.MinPolice) and hasgun() do
			sleep = 1500
			ply = GetPlayerPed(-1)
			plyloc = GetEntityCoords(ply)
			for i, v in pairs(Config.CaseLocations) do
				if GetDistanceBetweenCoords(plyloc, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.0  and not v.Broken and not IsBusy then
					...
					if IsControlJustPressed(0, 38) and not IsBusy and not IsPedWalking(ply) and not IsPedRunning(ply) and not IsPedSprinting(ply) then
						...
						TaskPlayAnim(ply, 'missheist_jewel', 'smash_case', 8.0, 1.0, -1, 2, 0, 0, 0, 0)
