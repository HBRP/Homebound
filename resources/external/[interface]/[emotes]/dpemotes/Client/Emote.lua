...
Citizen.CreateThread(function()
    ...
    if Config.SqlKeybinding then
      TriggerEvent('chat:addSuggestion', '/emotebind', 'Bind an emote', {{ name="key", help="num6, num7, num8. num9, num10. Numpad 6-0!"}, { name="emotename", help="dance, camera, sit or any valid emote."}})
...
function EmoteCommandStart(source, args, raw)
    ...
    if name == "c" then
        if IsInAnimation then
          ...
          exports['t-notify']:Alert({style = 'error', message = Config.Languages[lang]['nocancel']})
        end
      return
    ...
    elseif DP.PropEmotes[name] ~= nil then
      ...
      exports['t-notify']:Alert({style = 'error', message = string.format("%s is not a valid emote", name)})
    end
  end
end
