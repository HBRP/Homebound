...
RegisterServerEvent("cad:hotKeyOpen")
AddEventHandler("cad:hotKeyOpen", function()
...
RegisterServerEvent("cad:getOffensesAndOfficer")
AddEventHandler("cad:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	MySQL.Async.fetchAll('SELECT * FROM fine_types', {
		...
		TriggerClientEvent("cad:returnOffensesAndOfficer", usource, charges, officer)
	end)
end)

RegisterServerEvent("cad:performOffenderSearch")
AddEventHandler("cad:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE LOWER(`firstname`) LIKE @query OR LOWER(`lastname`) LIKE @query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE @query", {
		...
		TriggerClientEvent("cad:returnOffenderSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("cad:getOffenderDetails")
AddEventHandler("cad:getOffenderDetails", function(offender)
	local usource = source
	...
	TriggerClientEvent("cad:returnOffenderDetails", usource, offender)
end)

RegisterServerEvent("cad:getOffenderDetailsById")
AddEventHandler("cad:getOffenderDetailsById", function(char_id)
	local usource = source
	...
	if not offender then
		TriggerClientEvent("cad:closeModal", usource)
		TriggerClientEvent("cad:sendNotification", usource, "This person no longer exists.")
		return
	end
...
RegisterServerEvent("cad:saveOffenderChanges")
AddEventHandler("cad:saveOffenderChanges", function(id, changes, identifier)
	local usource = source
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		TriggerClientEvent("cad:sendNotification", usource, "Offender changes have been saved.")
	end)
end)

RegisterServerEvent("cad:saveReportChanges")
AddEventHandler("cad:saveReportChanges", function(data)
	MySQL.Async.execute('UPDATE `mdt_reports` SET `title` = @title, `incident` = @incident WHERE `id` = @id', {
		['@id'] = data.id,
		['@title'] = data.title,
		['@incident'] = data.incident
	})
	TriggerClientEvent("cad:sendNotification", source, "Report changes have been saved.")
end)

RegisterServerEvent("cad:deleteReport")
AddEventHandler("cad:deleteReport", function(id)
	MySQL.Async.execute('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		['@id']  = id
	})
	TriggerClientEvent("cad:sendNotification", source, "Report has been successfully deleted.")
end)

RegisterServerEvent("cad:submitNewReport")
AddEventHandler("cad:submitNewReport", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`) VALUES (@id, @title, @incident, @charges, @author, @name, @date)', {
		...
		TriggerEvent("cad:getReportDetailsById", id, usource)
		TriggerClientEvent("cad:sendNotification", usource, "A new report has been submitted.")
	end)
...
RegisterServerEvent("cad:performReportSearch")
AddEventHandler("cad:performReportSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		...
		TriggerClientEvent("cad:returnReportSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("cad:performVehicleSearch")
AddEventHandler("cad:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE LOWER(`plate`) LIKE @query", {
		...
		TriggerClientEvent("cad:returnVehicleSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("cad:performVehicleSearchInFront")
AddEventHandler("cad:performVehicleSearchInFront", function(query)
	local usource = source
    	...
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		...
    		MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE `plate` = @query", {
					...
					local officer = GetCharacterName(usource)
    				TriggerClientEvent('mdt:toggleVisibilty', usource, reports, warrants, officer, xPlayer.job.name)
					TriggerClientEvent("cad:returnVehicleSearchInFront", usource, result, query)
				end)
    		end)
    	end)
	end
...
RegisterServerEvent("cad:getVehicle")
AddEventHandler("cad:getVehicle", function(vehicle)
	local usource = source
	...
	TriggerClientEvent("cad:returnVehicleDetails", usource, vehicle)
end)

RegisterServerEvent("cad:getWarrants")
AddEventHandler("cad:getWarrants", function()
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		...
		TriggerClientEvent("cad:returnWarrants", usource, warrants)
	end)
end)

RegisterServerEvent("cad:submitNewWarrant")
AddEventHandler("cad:submitNewWarrant", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		TriggerClientEvent("cad:completedWarrantAction", usource)
		TriggerClientEvent("cad:sendNotification", usource, "A new warrant has been created.")
	end)
end)

RegisterServerEvent("cad:deleteWarrant")
AddEventHandler("cad:deleteWarrant", function(id)
	local usource = source
	...
	TriggerClientEvent("cad:sendNotification", usource, "Warrant has been successfully deleted.")
end)

RegisterServerEvent("cad:getReportDetailsById")
AddEventHandler("cad:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		...
		if result and result[1] then
			...
			TriggerClientEvent("cad:returnReportDetails", usource, result[1])
		else
			TriggerClientEvent("cad:closeModal", usource)
			TriggerClientEvent("cad:sendNotification", usource, "This report cannot be found.")
		end
	end)
end)

RegisterServerEvent("cad:newCall")
AddEventHandler("cad:newCall", function(details, caller, coords)
	...
	for i= 1, #xPlayers do
		...
		local xPlayer = ESX.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("cad:newCall", source, details, caller, coords, call_index)
...
RegisterServerEvent("cad:attachToCall")
AddEventHandler("cad:attachToCall", function(index)
	local usource = source
	...
	for i= 1, #xPlayers do
		...
		local xPlayer = ESX.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("cad:newCallAttach", source, index, charname)
		end
	end
	TriggerClientEvent("cad:sendNotification", usource, "You have attached to this call.")
end)

RegisterServerEvent("cad:detachFromCall")
AddEventHandler("cad:detachFromCall", function(index)
	local usource = source
	local charname = GetCharacterName(usource)
	local xPlayers = ESX.GetPlayers()
	for i= 1, #xPlayers do
		...
		local xPlayer = ESX.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("cad:newCallDetach", source, index, charname)
		end
	end
	TriggerClientEvent("cad:sendNotification", usource, "You have detached from this call.")
end)

RegisterServerEvent("cad:editCall")
AddEventHandler("cad:editCall", function(index, details)
	...
	for i= 1, #xPlayers do
		...
		local xPlayer = ESX.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("cad:editCall", source, index, details)
		end
	end
	TriggerClientEvent("cad:sendNotification", usource, "You have edited this call.")
end)

RegisterServerEvent("cad:deleteCall")
AddEventHandler("cad:deleteCall", function(index)
	...
	for i= 1, #xPlayers do
		...
		local xPlayer = ESX.GetPlayerFromId(source)
		if xPlayer.job.name == 'police' then
			TriggerClientEvent("cad:deleteCall", source, index)
		end
	end
	TriggerClientEvent("cad:sendNotification", usource, "You have deleted this call.")
end)

RegisterServerEvent("cad:saveVehicleChanges")
AddEventHandler("cad:saveVehicleChanges", function(data)
	...
	MySQL.Async.fetchAll('SELECT * FROM `vehicle_mdt` WHERE `plate` = @plate', {
		...
		TriggerClientEvent("cad:sendNotification", usource, "Vehicle changes have been saved.")
	end)
end)
