...
$(document).ready(function() {
    ...
    $("body").on("keyup", function (key) {
        if (closeKey.includes(key.which)) {
            $(".navigation").fadeOut();
            $.post(`http://amp-uimenu/closeUI`, JSON.stringify({value: false}));
        }
    });
    ...
    $("#backbtn").on("click", function(event) {
        $(".navigation").fadeOut();
        $(".mainmenu").empty();
        $.post(`http://amp-uimenu/selectedValue`, JSON.stringify({value: "back"}));
    })
    ...
    $(".mainmenu").on("click", ".item", function(event) {
        ...
        if (menuType == "list" || menuType == "hover") {
            ...
            $.post(`http://amp-uimenu/selectedValue`, JSON.stringify({value: (typeof thisval == "object") ? JSON.parse(thisval) : thisval}));
        }
        ...
        if (menuType == "multi") {
            ...
            let thisval = (typeof $(this).data("value") == "number") ? $(this).data("value") : window.atob($(this).data("value"))
            $.post(`http://amp-uimenu/hoveredValue`, JSON.stringify({value: (typeof thisval == "object") ? JSON.parse(thisval) : thisval, event: $(this).data("event")}));
        }
...
function buildMultiMenu(data) {
    ...
    for (let result of myMenu) {
        if (result.submenu) {
            let submenu = `<li><a href="" class="item">${result.submenuLabel}</a><ul class="submenu">`
            for (let subitem of result.submenu) {
                submenu = submenu + `<li><a href="" class="item" data-event="${result.event}" data-value="${(typeof subitem.value == "object") ? window.btoa(JSON.stringify(subitem.value)) : window.btoa(subitem.value)}">${subitem.hasitem ? '<i class="fas fa-check green"></i>&nbsp;' : '<i class="fas fa-times red"></i> '}${subitem.label}</a></li>`
            }
            submenu = submenu + `</ul></li>`
            $(".mainmenu").append(submenu)
        } else {
            $(".mainmenu").append(`<li><a href="" class="item" data-event="${data.event}" data-value="${(typeof result.value == "object") ? window.btoa(JSON.stringify(result.value)) : window.btoa(result.value)}">${result.hasitem ? '<i class="fas fa-check green"></i>&nbsp;' : '<i class="fas fa-times red"></i> '}${result.label}</a></li>`)
        }
    }
}
